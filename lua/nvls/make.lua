...
local function commands()
    ...
    local cmds = {
        ...
        lualatex = {
            efm = "%f:%l:%m,%-G%.%#",
            make = string.format(
                'lualatex --file-line-error --output-directory=%s --shell-escape --interaction=nonstopmode %s', C.folder,
                C.main)
        },
        texinfo = {
            efm = "%f:%l:%m,%-G%.%#",
        ...
        lilypondBook = {
            ...
            make = string.format('lilypond-book %s %s %s --output=%s %s', C.lb_flags, C.backend, C.include, C.tmp,
                vim.fn.expand(C.main))
        },
        lytex = {
            efm = "%f:%l:%m,%-G%.%#",
            make = string.format(
                'cd %s && lualatex --file-line-error --output-directory=%s --shell-escape --interaction=nonstopmode %s',
                C.tmp, C.folder, Utils.shellescape(Utils.joinpath(C.tmp, name .. '.tex'), true))
        },
        lytexi = {
            efm = "%f:%l:%m,%-G%.%#",
            make = string.format('cd %s && texi2pdf --output=%s %s', C.tmp, Utils.change_extension(C.main, "pdf"),
                Utils.shellescape(Utils.joinpath(C.tmp, name .. '.texi'), true))
        },
        fluidsynth = {
            efm = " ",
            make = (function()
                if C.midi_synth == "timidity" then
                    ...
                    return string.format('fluidsynth %s -T raw -F - %s -s | ffmpeg -f s32le -i - %s', C.fluidsynth_flags,
                        C.midi, C.audio)
                end
            end)()
        },
        tmpplayer = {
            ...
            make = (function()
                if C.midi_synth == "timidity" then
                    return string.format('timidity %s %s -Ow -o %s', Utils.joinpath(C.tmp, "tmp.midi"), C.timidity_flags,
                        Utils.joinpath(C.tmp, "tmp.wav"))
                else
                    return string.format('fluidsynth %s -T raw -F - %s -s | ffmpeg -f s32le -i - %s', C.fluidsynth_flags,
                        Utils.joinpath(C.tmp, "tmp.midi"), Utils.joinpath(C.tmp, "tmp." .. C.audio_format))
                end
            end)()
        },
    }
    ...
    local win_cmds = {
        lytex = {
            make = string.format(
                'cd /d %s & lualatex --file-line-error --output-directory=%s --shell-escape --interaction=nonstopmode %s',
                C.tmp, C.folder, Utils.shellescape(Utils.joinpath(C.tmp, name .. '.tex'), true))
        },
        fluidsynth = {
            make = string.format('timidity %s %s -Ow -o %s', Utils.joinpath(C.tmp, "tmp.midi"), C.timidity_flags,
                Utils.joinpath(C.tmp, "tmp." .. C.audio_format))
        },
        tmpplayer = {
            make = string.format('timidity %s %s -Ow -o %s', Utils.joinpath(C.tmp, "tmp.midi"), C.timidity_flags,
                Utils.joinpath(C.tmp, "tmp." .. C.audio_format))
